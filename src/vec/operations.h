...
// This function picks one bit from pickNumber and places it at the least significant position of
// number.
[[nodiscard]] std::vector<std::uint8_t> addBitFromNumber(
    ...
    // one vector element with value 1
...
[[nodiscard]] std::vector<std::uint8_t> div(
    ...
    while (dividendIndex >= -1) {
        ...
        if (isEqual(dividendMask, divisor) || isBigger(dividendMask, divisor)) {
            // Stop the loop if the dividend is smaller than the divisor, because fractional digits
            // are not supported
            if (dividendIndex < 0) {
                quotientBuffer <<= 1;
            ...
            // if the divisor is bigger than the dividend, we need to shift the dividend and set the
            // new bit as low
            quotientBuffer <<= 1;
            quotientBitIndex++;
